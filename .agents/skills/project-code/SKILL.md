...
理解特性需求并根据下述规范进行开发。开发完成后，通过subagent调用``reviewer``技能进行代码检视，并根据review.log中的检视意见进行处理。如果review.log开头已经写“检视完成，功能精度确认ok”，则结束检视进行提交；否则，继续修改并通过subagent调用``reviwer``技能进行检视，重复直到功能精度ok。
