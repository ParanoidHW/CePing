...
from llm_perf.modeling.layers import ShardedTensor
...
def attention_layer(
    ...
    q_layer, q_result = linear_layer(
        hidden_size, hidden_size, batch_size, seq_len, bias=False, dtype=dtype, name=f"{name}_q"
    )
    ...
    k_layer, k_result = linear_layer(
        kv_in_features, kv_hidden_size, batch_size, kv_len, bias=False, dtype=dtype, name=f"{name}_k"
    )
    layers.append((k_layer, k_result))

    v_layer, v_result = linear_layer(
        kv_in_features, kv_hidden_size, batch_size, kv_len, bias=False, dtype=dtype, name=f"{name}_v"
    )
    ...
    if use_flash_attention:
        attn_result = flash_attention(q_shape, k_shape, v_shape, is_causal=is_causal, dtype=dtype, use_gqa=use_gqa)
    else:
        attn_result = scaled_dot_product_attention(q_shape, k_shape, v_shape, dtype=dtype, use_gqa=use_gqa)
    ...
    o_layer, o_result = linear_layer(
        hidden_size, hidden_size, batch_size, seq_len, bias=False, dtype=dtype, name=f"{name}_o"
    )
...
def mla_attention_layer(
    ...
    q_down_layer, q_down_result = linear_layer(
        hidden_size, kv_lora_rank, batch_size, seq_len, bias=False, dtype=dtype, name=f"{name}_q_down"
    )
    ...
    if not use_absorb:
        ...
        kv_down_layer, kv_down_result = linear_layer(
            hidden_size, kv_lora_rank, batch_size, seq_len, bias=False, dtype=dtype, name=f"{name}_kv_down"
        )
        ...
        k_up_layer, k_up_result = linear_layer(
            kv_lora_rank, k_up_dim, batch_size, seq_len, bias=False, dtype=dtype, name=f"{name}_k_up"
        )
        ...
        v_up_layer, v_up_result = linear_layer(
            kv_lora_rank, v_up_dim, batch_size, seq_len, bias=False, dtype=dtype, name=f"{name}_v_up"
        )
    ...
    # Output projection
    o_layer, o_result = linear_layer(
        num_heads * v_head_dim, hidden_size, batch_size, seq_len, bias=False, dtype=dtype, name=f"{name}_o"
    )
    layers.append((o_layer, o_result))
...
def ffn_layer(
    ...
    if gated:
        ...
        in_layer, in_result = linear_layer(
            hidden_size, intermediate_size * 2, batch_size, seq_len, bias=False, dtype=dtype, name=f"{name}_in"
        )
        ...
        # Standard FFN
        in_layer, in_result = linear_layer(
            hidden_size, intermediate_size, batch_size, seq_len, bias=True, dtype=dtype, name=f"{name}_in"
        )
        layers.append((in_layer, in_result))
    ...
    out_layer, out_result = linear_layer(
        intermediate_size, hidden_size, batch_size, seq_len, bias=True, dtype=dtype, name=f"{name}_out"
    )
...
def transformer_block(
    ...
    self_attn_layers = attention_layer(
        batch_size, seq_len, num_heads, hidden_size // num_heads, dtype=dtype, name=f"{name}_self_attn"
    )
    ...
    ffn_layers = ffn_layer(
        batch_size, seq_len, hidden_size, intermediate_size, gated=gated_ffn, dtype=dtype, name=f"{name}_ffn"
    )
